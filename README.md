...
*   **Tab 自动补全**: 支持 Tab 键自动补全命令、参数和设备分区路径。输入部分命令后按 Tab 键即可自动补全或显示匹配选项。内置模糊匹配和拼写纠正功能，即使输入有轻微错误也能智能识别。
...
*   **用法**: 输入 `theme` 回车进入主题选择界面，或使用 `theme <编号>` 直接切换主题。
*   **特性**: 每种主题都有独特的配色方案，配置会自动保存，下次启动时自动应用。主题影响命令行提示符的颜色和样式。
...
*   **用法**: 在交互式命令行中输入 `shizuku` 回车。
*   **功能**: 自动检测设备上的 Shizuku 应用安装状态，并执行激活命令，方便使用 Shizuku 提供的 API 权限。
